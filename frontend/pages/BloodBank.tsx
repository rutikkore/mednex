...
const BloodBank: React.FC = () => {
  ...
  useEffect(() => {
    const fetchBloodBanks = async () => {
      ...
      if (error) console.error('Error fetching blood banks:', error);
      else if (data) {
        const mappedBanks: BloodBankData[] = data.map((b: any) => ({
          id: b.id,
          name: b.name,
        ...
        if (mappedBanks.length > 0) setSourceBankId(mappedBanks[0].id);
      }
      setLoading(false);
    };
    fetchBloodBanks();
            ...
            <div className="glass-premium rounded-[3rem] p-10 sm:p-12 border border-white/10 shadow-3xl">
              <div className="space-y-10">
                  ...
                  <div className="grid grid-cols-4 gap-3">
                    {BLOOD_GROUPS.map(g => (
                      <button
                        ...
                        className={`py-4 rounded-[1.2rem] text-[10px] font-black uppercase tracking-[0.1em] transition-all border ${selectedGroup === g
                ...
                <button
                  ...
                  className={`w-full py-7 rounded-full font-black text-xs uppercase tracking-[0.4em] shadow-2xl transition-all duration-500 active:scale-95 flex items-center justify-center gap-4 group ${isOutOfStock ? 'bg-red-600 hover:bg-white hover:text-black' : 'bg-blue-600 hover:bg-white hover:text-black'
                    }`}
                >
                  {isSearching ? (
