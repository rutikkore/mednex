...
import { INDIAN_HOSPITALS, BLOOD_BANKS, BLOOD_GROUPS } from '../constants';
...
const AdminDashboard: React.FC = () => {
  ...
  const fetchLiveData = () => {
    setTokens(JSON.parse(localStorage.getItem('smartflow_db_tokens') || '[]'));
    setBloodRequests(JSON.parse(localStorage.getItem('smartflow_db_blood_requests') || '[]'));
  };

  useEffect(() => { fetchLiveData(); }, []);
  useEffect(() => { if (lastEvent) fetchLiveData(); }, [lastEvent]);

  // Comprehensive Analytics Calculations
  const analytics = useMemo(() => {
    const totalIcuTotal = INDIAN_HOSPITALS.reduce((acc, h) => acc + h.beds.icu.total, 0);
    const totalIcuAvail = INDIAN_HOSPITALS.reduce((acc, h) => acc + h.beds.icu.available, 0);
    const totalGenTotal = INDIAN_HOSPITALS.reduce((acc, h) => acc + h.beds.general.total, 0);
    const totalGenAvail = INDIAN_HOSPITALS.reduce((acc, h) => acc + h.beds.general.available, 0);
    
    // Calculate global blood group distribution
    const globalBloodStock = BLOOD_GROUPS.map(group => ({
      name: group,
      value: BLOOD_BANKS.reduce((acc, bank) => acc + (bank.stock[group] || 0), 0)
    }));
    ...
    return {
      icuUtilization: Math.round(((totalIcuTotal - totalIcuAvail) / totalIcuTotal) * 100),
      genUtilization: Math.round(((totalGenTotal - totalGenAvail) / totalGenTotal) * 100),
      globalBloodStock,
      totalHospitals: INDIAN_HOSPITALS.length,
      activeWaiters,
      emergencyCount,
      bloodTransitCount: bloodRequests.length,
      nodeStatus: 'STABLE'
    };
  }, [tokens, bloodRequests]);

  // Multi-series chart data for hospitals
  const hospitalPerformanceData = INDIAN_HOSPITALS.map(h => ({
    name: h.name.replace(' Hospital', '').replace(' & Research Centre', '').substring(0, 10),
    icu: h.beds.icu.available,
    general: h.beds.general.available / 10, // Scaled for comparison
    cardiac: h.beds.cardiac.available,
    total: h.beds.icu.total + h.beds.general.total + h.beds.cardiac.total
  }));
      ...
      <div className="w-full max-w-[1800px] mx-auto space-y-12 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-10 reveal active">
          ...
          <div className="flex flex-wrap gap-4">
             ...
             <div className="px-8 py-4 bg-blue-600 text-white rounded-[2rem] flex items-center gap-4 shadow-2xl shadow-blue-600/20 active:scale-95 cursor-pointer">
            ...
            <div className="glass-premium rounded-[3.5rem] border border-white/10 shadow-3xl p-10 sm:p-16 relative group">
               <div className="flex justify-between items-start mb-16">
                 ...
                 <div className="flex gap-3">
                   ...
                   <span className="px-4 py-2 bg-white/5 rounded-xl text-[9px] font-black uppercase text-emerald-500 border border-emerald-500/20">Cardiac</span>
                 </div>
               </div>
               ...
               <div className="h-[450px] w-full opacity-80">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={hospitalPerformanceData} barGap={8}>
                      ...
                      />
                      <Bar dataKey="icu" fill="#3b82f6" radius={[6, 6, 0, 0]} name="ICU Available" barSize={35} />
                      <Bar dataKey="cardiac" fill="#10b981" radius={[6, 6, 0, 0]} name="Cardiac Available" barSize={35} />
                    </BarChart>
                  </ResponsiveContainer>
               </div>
            </div>
                  ...
                  <div className="h-[200px] w-full opacity-60">
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={pulseTrends}>
                        ...
                        <Tooltip contentStyle={{ borderRadius: '20px', background: '#020617', border: 'none' }} />
            ...
            <div className="glass-premium rounded-[3rem] p-10 border border-white/10 shadow-3xl text-center">
               ...
               <div className="h-[280px] w-full relative">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                     ...
                     <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest mt-2">Units in Node</p>
                  </div>
               </div>
               <div className="grid grid-cols-4 gap-4 mt-8">
                  {analytics.globalBloodStock.slice(0, 4).map((g, i) => (
                    <div key={i} className="space-y-1">
                      <p className="text-[10px] font-black text-white">{g.name}</p>
                      <div className="w-full h-1 bg-white/5 rounded-full">
                        <div className="h-full bg-rose-500" style={{ width: `${(g.value / 100) * 100}%` }}></div>
                      </div>
                    </div>
                  ))}
               </div>
               ...
               <div className="flex-1 overflow-y-auto space-y-6 pr-2 custom-scrollbar">
                  {tokens.length > 0 ? tokens.slice(-15).reverse().map((log, i) => (
                    <div key={i} className="p-6 rounded-[2rem] bg-white/5 border border-white/5 group hover:bg-white/10 hover:border-blue-500/30 transition-all flex gap-5 items-start">
                       <div className="w-12 h-12 bg-blue-600/10 text-blue-500 rounded-2xl flex items-center justify-center shrink-0 border border-blue-500/20 group-hover:bg-blue-600 group-hover:text-white transition-all duration-500">
                          <span className="material-icons-round text-xl">token</span>
                       </div>
                       <div className="min-w-0">
                          <p className="font-black text-white text-sm uppercase tracking-tight truncate">{log.number} Registered</p>
                          <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-1 truncate">{log.hospitalName}</p>
                          <div className="flex items-center gap-3 mt-3">
                             <span className="text-[8px] font-black text-slate-600">{log.timestamp}</span>
