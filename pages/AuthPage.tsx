...
const AuthPage: React.FC = () => {
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      if (formData.email && formData.email.includes('@')) {
        const exists = checkEmail(formData.email);
        setIsNewUser(!exists);
  ...
  const handleAuthAction = async (e: React.FormEvent) => {
    ...
    if (isNewUser) {
      ...
      result = register(formData.name, formData.email, formData.password, formData.role);
    } else {
      result = login(formData.email, formData.password);
    }

    if (result.success) {
      const sessionData = localStorage.getItem('smartflow_session');
      if (sessionData) {
        const sessionUser = JSON.parse(sessionData);
        if (sessionUser.role === 'patient') navigate('/patient');
        else if (sessionUser.role === 'admin') navigate('/admin');
        else navigate('/staff');
      }
    } else {
      setErrorMsg(result.message || 'Node authentication failed.');
      setIsLoading(false);
    }
  };
  ...
  const expressAccess = (role: UserRole) => {
    ...
    setTimeout(() => {
      register(names[role], emails[role], 'password', role);
      navigate(role === 'patient' ? '/patient' : role === 'admin' ? '/admin' : '/staff');
    }, 500);
  };
      ...
      <div className="w-full max-w-xl relative z-10 space-y-12 animate-in fade-in zoom-in-95 duration-700">
        <div className="text-center space-y-4">
          <Link to="/" className="inline-flex flex-col items-center group">
            ...
            <div className="badge-yellow">Operational Entry Port</div>
          </Link>
        </div>
        ...
        <div className="glass-premium rounded-[4rem] p-10 lg:p-14 border border-white/10 shadow-3xl">
          <form onSubmit={handleAuthAction} className="space-y-8">
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-500 ml-4">Identifier</label>
              <input 
                required
                type="email" 
            ...
            {emailChecked && isNewUser && (
              <div className="space-y-8 animate-in slide-in-from-top-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-500 ml-4">Full Identity Name</label>
                  <input 
                    required={isNewUser}
                    type="text" 
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {(['patient', 'receptionist', 'admin'] as UserRole[]).map((r) => (
                      <button
                        ...
                        className={`py-4 rounded-3xl text-[9px] font-black uppercase tracking-[0.2em] transition-all border ${
                          formData.role === r 
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-500 ml-4">Secure Passkey</label>
              <input 
                required
                type="password" 
                placeholder="••••••••"
