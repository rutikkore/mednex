...
const EmergencyRedirect: React.FC = () => {
  const [currentHospitalId, setCurrentHospitalId] = useState(INDIAN_HOSPITALS[0].id);
  ...
  const currentHospital = INDIAN_HOSPITALS.find(h => h.id === currentHospitalId)!;

  const handleSearch = () => {
    setIsSearching(true);
    const alternatives = INDIAN_HOSPITALS
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {(['icu', 'cardiac', 'general'] as const).map(type => (
                      <button
                        ...
                        className={`py-5 rounded-[1.5rem] text-[10px] font-black uppercase tracking-[0.2em] transition-all border ${
                          requiredBedType === type 
